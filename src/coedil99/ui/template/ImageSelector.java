...
public class ImageSelector extends JComboBox{
	...
	public ImageSelector() {
		...
		images = new ImageIcon[elementi.length];
		Integer[] intArray = new Integer[elementi.length];
		for (int i = 0; i < elementi.length; i++) {
			intArray[i] = new Integer(i);
			images[i] = createImageIcon("../img/" + elementi[i] + ".gif");
			if (images[i] != null) {
				images[i].setDescription(elementi[i]);
			}
		}
	...
	class ComboBoxRenderer extends JLabel implements ListCellRenderer {
		...
		public Component getListCellRendererComponent(
			...
			String pet = elementi[selectedIndex];
			setIcon(icon);
			if (icon != null) {
				//setText(pet);
